...
public class Taulell {
    ...
    public Taulell(Jugador[] jugadors) {
        ...
        //  if (jugadors.length < Fartos_2086.MinJugadors)
        //      throw new IllegalArgumentException("No es pot jugar amb menys de 3 jugadors.");
        //  if (jugadors.length > Fartos_2086.MaxJugadors)
    ...
    public String toString() {
        ...
        for (int i = 0; i < LongTaulell; i++) {
            ...
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
                comprovacioEspecial = Arrays.stream(PosEspecial).filter(n -> n == finalI).findAny().orElse(-1);
            }
    ...
    public boolean comprobarCasellaEspecial(int casella) {
        int comprovacioEspecial =
                0;
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
            comprovacioEspecial = Arrays.stream(PosEspecial).filter(n -> n == casella).findAny().orElse(-1);
        }
    ...
    public boolean comprobarCasellaEspecial(Jugador jugador) {
        int comprovacioEspecial =
                0;
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
            comprovacioEspecial = Arrays.stream(PosEspecial).filter(n -> n == (int) taulell.get(jugador))
                    .findAny()
                    .orElse(-1);
        }
        if (comprovacioEspecial != -1) return true;
